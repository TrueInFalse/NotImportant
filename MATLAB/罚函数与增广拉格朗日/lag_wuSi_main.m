...
while k < 1000
    % 增广拉格朗日函数
    L = @(x, lambda, sigma) f(x) + lambda(1) * g1(x) + lambda(2) * g2(x) + lambda(3) * g3(x) + lambda(4) * g4(x) ...
                                  + (sigma / 2) * (g1(x)^2 + g2(x)^2 + g3(x)^2 + g4(x)^2);
    
    % 使用 fminunc 求解子问题
    options = optimoptions('fminunc', 'Display', 'off', 'Algorithm', 'quasi-newton');
    [x_k1, fval] = fminunc(@(x) L(x, lambda0, sigma0), x0, options);
    
    % 计算约束违反度
    omega = norm([g1(x_k1), g2(x_k1), g3(x_k1), g4(x_k1)]);
    
    % 检查精度条件 ||∇L(x^(k+1))|| <= η_k
    grad_L = gradient_L(x_k1, lambda0, sigma0);  % 计算梯度
    grad_norm = norm(grad_L);
    
    if grad_norm <= eta_k  % 子问题求解误差满足条件
        if omega <= epsilon  % 约束违反度满足条件
            % 更新乘子
            lambda0 = lambda0 + sigma0 * [g1(x_k1), g2(x_k1), g3(x_k1), g4(x_k1)];
            sigma0 = sigma0 * rho;
            eta_k = eta_k / rho;
            epsilon = epsilon / rho;
