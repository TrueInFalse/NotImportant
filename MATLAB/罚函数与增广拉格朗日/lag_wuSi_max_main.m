...
while k < 1000
    ...
    L = @(x, mu) f(x) + sum(mu .* max(0, [g1(x), g2(x), g3(x), g4(x)])') + ...
                 (sigma0 / 2) * sum(max(0, [g1(x), g2(x), g3(x), g4(x)]).^2);
    
    % 使用 fminunc 求解子问题
    options = optimoptions('fminunc', 'Display', 'off', 'Algorithm', 'quasi-newton');
    [x_k1, fval] = fminunc(@(x) L(x, mu0), x0, options);
    
    % 计算约束违反度
    omega = norm([g1(x_k1), g2(x_k1), g3(x_k1), g4(x_k1)]);

    % 检查精度条件 ||∇L(x^(k+1))|| <= η_k
    grad_L = gradient_L(x_k1, mu0, sigma0);  % 计算梯度
    grad_norm = norm(grad_L);
    
    if grad_norm <= eta_k  % 子问题求解误差满足条件
        if omega <= epsilon  % 约束违反度满足条件
            % 更新乘子
            mu0 = mu0 + sigma0 * max(0, [g1(x_k1), g2(x_k1), g3(x_k1), g4(x_k1)]);
            sigma0 = sigma0 * rho;
            eta_k = eta_k / rho;
            epsilon = epsilon / rho;
